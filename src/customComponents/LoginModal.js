...
export default function LoginModal({ show, onHide, initialTab = 'login', initialReferralCode = '', returnTo = '' }) {
  ...
  // Lock page scroll while modal open (mobile: stops background scrolling / iOS bounce)
  useEffect(() => {
  ...
  // Har baar modal khule: scroll top pe (mobile pe logo / close dikhe)
  useEffect(() => {
    if (!show) return
    const el = loginScrollRef.current
    if (el) el.scrollTop = 0
    requestAnimationFrame(() => {
      if (loginScrollRef.current) loginScrollRef.current.scrollTop = 0
    })
  }, [show, activeTab, showForgotPassword])
  ...
  return (
    ...
    >
      ...
      >
        <div ref={loginScrollRef} className="login_modal_content premium_login_content">
          <button
            type="button"

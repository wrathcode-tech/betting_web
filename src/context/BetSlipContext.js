...
export function BetSlipProvider({ children }) {
  ...
  const addSelection = useCallback((selection) => {
    const s = {
      ...
      ...(selection.seriesName ? { seriesName: selection.seriesName } : {}),
  ...
  const placeBet = useCallback(async () => {
    ...
    try {
      ...
      const body = {
        sport: sel.sport,
        gameId: String(sel.gameId),
        eventName: sel.eventName,
        ...(sel.seriesName ? { seriesName: sel.seriesName } : {}),
        ...(sel.eventTime ? { eventTime: sel.eventTime } : {}),
        marketType: sel.marketType,
        marketId: String(sel.marketId),
        ...(sel.marketName ? { marketName: sel.marketName } : {}),
        selectionId: String(sel.selectionId),
      ...
      const { ok, balanceAfter, message: failMsg } = sportsbookApi.unwrapPlaceBetResponse(res);

      if (ok) {
        if (balanceAfter != null) setBalance(balanceAfter);
      ...
      const errorMsg = isDemo
        ...
        : (failMsg || res?.message || 'Insufficient balance');
      setSlip((prev) => ({
        ...prev,
        placing: false,
        placeError: errorMsg,
